...
export const LandingPage = () => {
  ...
  // outline-кнопка под светлый хедер
  const landingButtonLight =
  ...
  // outline-кнопка под тёмный hero
  const landingButtonDark =
    'rounded-none border border-white/60 bg-transparent text-white hover:border-accent hover:bg-transparent hover:text-accent';
  ...
  return (
    <main className="bg-white text-primary">
      {/* HERO */}
      <div className="relative min-h-screen isolate overflow-hidden bg-[#0b0620]">
        <AnimatedBackground />

        {/* Header */}
        <header className="sticky top-0 z-20 border-b border-white/10 bg-white/80 backdrop-blur-sm">
          <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
            <span className="text-lg font-semibold tracking-tight">BioEQ</span>

            <nav className="hidden items-center gap-6 text-sm sm:flex">
              <a href="#examples" className="text-primary/75 transition hover:text-primary">
                Примеры
              </a>
              <a href="#team" className="text-primary/75 transition hover:text-primary">
                Команда
              </a>
              <a href="#architecture" className="text-primary/75 transition hover:text-primary">
                Архитектура
              </a>
              <a href="#implementation" className="text-primary/75 transition hover:text-primary">
                Внедрение
              </a>
            </nav>

            <Button onClick={goToForm} className={`w-full sm:w-auto ${landingButtonLight}`}>
              Старт
            </Button>
          </div>
        </header>
          ...
          <div className="relative z-10 mx-auto flex min-h-[calc(100vh-73px)] w-full max-w-6xl items-center px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
            ...
            >
              <motion.h1
                ...
                className="text-5xl font-semibold leading-none tracking-tight text-white sm:text-6xl lg:text-7xl"
              >
                BioEQ
              </motion.h1>

              <motion.p variants={itemVariants} className="mt-5 text-lg text-white/85 sm:text-xl">
                AI-инструмент для автоматизированного проектирования исследований биоэквивалентности
              </motion.p>

              <motion.p variants={itemVariants} className="mx-auto mt-5 max-w-2xl text-base text-white/70 lg:mx-0 lg:text-lg">
                Вы заполняете параметры препарата, а наши ИИ-агенты делают всё остальное
              </motion.p>

              <motion.div variants={itemVariants} className="mt-10">
                <Button onClick={goToForm} className={`w-full sm:w-auto sm:px-8 sm:py-4 sm:text-base ${landingButtonDark}`}>
                  Старт
                </Button>
              </motion.div>

              <motion.div variants={itemVariants} className="mt-10 flex flex-wrap items-center justify-center gap-3 lg:justify-start">
                <span className="rounded-none border border-white/15 bg-white/5 px-3 py-1 text-xs text-white/80 backdrop-blur">
                  Future AI MedTech
                </span>
                <span className="rounded-none border border-white/15 bg-white/5 px-3 py-1 text-xs text-white/80 backdrop-blur">
                  Быстро
      ...
      <section
        id="examples"
        className="mx-auto w-full max-w-6xl scroll-mt-24 overflow-x-clip bg-primary px-4 py-16 text-white sm:px-6 lg:px-8 lg:py-24"
      >
        ...
        >
          {loopedExampleCards.map((card, index) => (
            <motion.article
              ...
              transition={{ duration: 0.45, delay: Math.min(index, 6) * 0.08, ease: 'easeOut' }}
            ...
            >
              ...
              <Button className="mt-6 rounded-none border border-white/45 bg-white/10 text-white hover:border-accent hover:bg-transparent hover:text-accent">
                Скачать
              </Button>
            </motion.article>
          ))}
      ...
      <section
        id="architecture"
        className="mx-auto w-full max-w-6xl scroll-mt-24 bg-primary px-4 py-16 text-white sm:px-6 lg:px-8 lg:py-24"
      >
        ...
        <div className="mt-10 space-y-8 border-t border-white/20 pt-8">
          {storySteps.map((step) => (
            <article
              key={step.number}
              className="grid gap-3 border-b border-white/20 pb-7 last:border-0 last:pb-0 md:grid-cols-[72px_1fr]"
            >
      ...
      <section
        id="implementation"
        className="mx-auto w-full max-w-6xl scroll-mt-24 px-4 py-16 text-primary sm:px-6 lg:px-8 lg:py-24"
      >
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6 lg:px-8 lg:pb-24">
        <div className="border border-primary/10 bg-neutral-50 px-6 py-10 text-center sm:px-8">
          ...
          <Button onClick={goToForm} className={`mt-7 w-full sm:w-auto ${landingButtonLight}`}>
            Перейти к форме
          </Button>
        </div>
      </section>
